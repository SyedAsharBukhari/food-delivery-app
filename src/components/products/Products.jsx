...
import { addtocart, wishlistdata } from '../../Store/slice/ProductSlice'
...
const data = products.map((news) => {
  // console.log(news)
  return {...news, quantity: 1}
...
// useEffect(() => {
//   if (wishlist) {
//     dispatch(wishlistdata(wishlist));
//   }
// }, [wishlist]);
...
return (
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {data.map((item) => (
            <div
              key={item.id}
              className="bg-white relative shadow-lg rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300"
            >
              <img
                src={item.thumbnail}
                alt={item.title}
                className="w-full h-48 object-cover"
              />
              {/* <FaRegHeart className='text-2xl absolute top-4 right-3' onClick={() => setwishlist( item)}/> */}
             <FaRegHeart
  className={`text-2xl absolute top-4 right-3 cursor-pointer ${wishlists.find((data) => data.id === item.id) ? "text-red-500" : ""}`}
  onClick={() => dispatch(wishlistdata(item))}
/>
              <RiShoppingCart2Line className='text-2xl absolute top-12 right-3 cursor-pointer'  onClick={() => dispatch(addtocart(item))}/>
              <div className="p-5">
                <h3 className="text-lg font-semibold text-gray-800">
                  {item.title}
                </h3>
